/*
** RocksDB Performance Benchmark - Optimized for Small DB
** 
...
static void configure_small_db_options(Options &options) {
    ...
    // Block cache: 2MB (matching SQLite 2000 pages × 1KB)
    ...
    // Disable WAL fsync for better write performance (matching KVStore's behavior)
    // KVStore uses autocommit transactions which are similar
...
static void bench_exists_checks(DB *db) {
    ...
    for (i = 0; i < NUM_READS; i++) {
        ...
        // Just check existence, don't care about the value
    }
